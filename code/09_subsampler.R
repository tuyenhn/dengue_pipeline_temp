...
required_packages <- c(
  "optparse", "dplyr", "lubridate", "tidyr", "anytime",
...
metadata_df <- metadata_df %>%
  ...
  mutate(Month = month(floor_date(Date, "month"))) %>%
  mutate(Week = week(floor_date(Date, "week")))
...
column_of_interest <- sym(colnames(location_of_interest)[1])
values_of_interest <- location_of_interest %>% pull({{ column_of_interest }})

info_msg("Column of interest: ", column_of_interest)
info_msg("Values of interest: ", paste0(values_of_interest, sep = ", "))

# add time interval
time_interval <- sym(opt[["time_interval"]])
...
##############################################################################
# Step 3: Select sample from Location of Interest using chosen sampling method
##############################################################################
...

{
  ...
  plot_dat_1 <- metadata_countries_of_interest %>%
    group_by({{ time_interval }}) %>%
    summarise(count = n()) %>%
    ungroup()

  plot_1 <- ggplot(plot_dat_1, aes(x = {{ time_interval }}, y = count)) +
  ...
  # save as pdf
  ggsave(
    filename = paste0(opt[["output_dir"]], "metadata_countries_of_interest_plot_", Serotype, ".pdf"),
    plot = plot_1
  )
}
...
df_sample <- if (no_seq_local == "all" || is.null(no_seq_local)) {
  ...
  # even weighted sampling method for local sequence
  info_msg(sprintf(
...
plot_2 <- ggplot(df_sample_location_of_interest_plot, aes(x = {{ time_interval }}, y = count, fill)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(x = "Year", y = "Number of Sequences", title = paste0("Number of Sequences per Year in ", paste0(values_of_interest, collapse = ";"))) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
# save as pdf
ggsave(
  filename = paste0(opt[["output_dir"]], "df_sample_location_of_interest_plot_", Serotype, ".pdf"),
  plot = plot_2
)
...
if (opt[["sampling_method"]] == "even") {
  ...
  # save as pdf
  ggsave(
    filename = paste0(opt[["output_dir"]], "background_metadata_plot_even_", Serotype, ".pdf"),
    plot = plot_3
  )
  ...
  # save as pdf
  ggsave(
    filename = paste0(opt[["output_dir"]], "background_sample_plot_even_", Serotype, ".pdf"),
    plot = plot_4
  )
}
...
if (opt[["sampling_method"]] == "proportional") {
  ...
  # save as pdf
  ggsave(
    filename = paste0(opt[["output_dir"]], "background_metadata_plot_proportional_", Serotype, ".pdf"),
    plot = plot_5
  )
  ...
  # save as pdf
  ggsave(
    filename = paste0(opt[["output_dir"]], "background_sample_plot_proportional_", Serotype, ".pdf"),
    plot = plot_6
  )
}
...
print(colnames(df_sample_location_of_interest))
print(colnames(background_sample_even))

if (opt[["sampling_method"]] == "even") {
  final_sample <- rbind(
    df_sample_location_of_interest,
    background_sample_even
  )
} else {
  final_sample <- rbind(
    df_sample_location_of_interest,
    background_sample_proportional
  )
}
