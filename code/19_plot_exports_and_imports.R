...
required_packages <- c(
  "optparse", "tidyverse", "janitor", "lubridate",
  "stringr", "patchwork", "NatParksPalettes", "RColorBrewer",
  "ape", "phylobase", "scales", "readr"
)
...
total_imports <- ggplot(plot_data_2, aes(x = head_country, y = count, fill = count)) +
  ...
  labs(
    y = "No. of inferred imports", x = "Source country", fill = "Number of Imports",
...
ggsave(
  filename = paste0(opt[["output_dir"]], "total_imports_", serotype, ".pdf"),
  plot = total_imports
)
...
imports_over_time <- ggplot(plot_data_over_time, aes(x = Year, y = count, fill = count)) +
  geom_col(position = position_dodge2(width = 0.9, preserve = "single")) +
  labs(
    y = "No. of inferred imports", x = "Date", fill = "Number of Imports",
    title = paste0("DENV imports to ", country)
  ) +
  theme_minimal() +
  scale_fill_gradient(low = "lightblue", high = "darkblue")
...
ggsave(
  filename = paste0(opt[["output_dir"]], "total_imports_over_time", serotype, ".pdf"),
  plot = imports_over_time
)
...
total_exports <- ggplot(plot_data_3, aes(x = tail_country, y = count, fill = count)) +
  geom_col(position = position_dodge2(width = 0.9, preserve = "single")) +
  labs(
    y = "No. of inferred exports", x = "Source country", fill = "Number of export",
...
ggsave(
  filename = paste0(opt[["output_dir"]], "total_export_", serotype, ".pdf"),
  plot = total_exports
)
...
exports_over_time <- ggplot(plot_data_over_time_exports, aes(x = Year, y = count, fill = count)) +
  geom_col(position = position_dodge2(width = 0.9, preserve = "single")) +
  labs(
    y = "No. of inferred export", x = "Date", fill = "Number of export",
    title = paste0("DENV export from ", country)
  ) +
  theme_minimal() +
  scale_fill_gradient(low = "lightblue", high = "darkblue")
...
ggsave(
  filename = paste0(opt[["output_dir"]], "total_exports_over_time", serotype, ".pdf"),
  plot = exports_over_time
)
